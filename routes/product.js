...
router.get('/produtos/editar/:id', (req, res) => {
  ...
  db.query(query, [id], (err, results) => {
    ...
    // LOGS de diagnóstico
    console.log('[EDIT PRODUTO] iniciar diagnóstico para ID:', id);
    console.log('[EDIT PRODUTO] produto (selecionado):', {
      ID: produto.ID,
      Nome: produto.nome,
      CategoriaProduto_ID: produto.CategoriaProduto_ID,
      Categoria_ID: produto.Categoria_ID,
      categoria_campos_raw: produto.categoria_campos,
      especificacoes_raw: produto.especificacoes
    });

    // normaliza campos vindos de CategoriasProduto (se existir)
    let camposCategoria = [];
    if (produto.categoria_campos) {
      try {
        ...
        console.log('[EDIT PRODUTO] parsed categoria_campos (CategoriasProduto):', parsed);
        ...
        console.error('Erro ao parsear categoria_campos (CategoriasProduto):', e);
        camposCategoria = [];
      }
    } else {
      console.log('[EDIT PRODUTO] produto.categoria_campos indefinido ou vazio para Produto.ID =', produto.ID);
    }

    // helper para renderizar com categorias do catálogo (tabela Categoria)
    function renderWithCatalogo(campos) {
      console.log('[EDIT PRODUTO] renderWithCatalogo - campos recebidos:', campos);
      db.query('SELECT * FROM Categoria', (errCat, categoriasCatalogo) => {
        ...
        res.render('editProduto', {
          ...
          categoriaCampos: campos || []
        });
      });
    }

    if (camposCategoria.length > 0) {
      console.log('[EDIT PRODUTO] usando campos da CategoriasProduto, total:', camposCategoria.length);
      return renderWithCatalogo(camposCategoria);
    }

    // fallback: busca campos na tabela Categoria usando Produto.Categoria_ID
    if (produto.Categoria_ID) {
      console.log('[EDIT PRODUTO] tentando fallback na tabela Categoria com Categoria_ID =', produto.Categoria_ID);
      db.query('SELECT campos FROM Categoria WHERE ID = ?', [produto.Categoria_ID], (errCatF, rowsCatF) => {
        console.log('[EDIT PRODUTO] resultado consulta Categoria (fallback):', { err: errCatF, rows: rowsCatF && rowsCatF[0] ? rowsCatF[0] : rowsCatF });
        if (!errCatF && rowsCatF && rowsCatF[0] && rowsCatF[0].campos) {
          try {
            const parsed2 = typeof rowsCatF[0].campos === 'string' ? JSON.parse(rowsCatF[0].campos) : rowsCatF[0].campos;
            console.log('[EDIT PRODUTO] parsed campos da Categoria (fallback):', parsed2);
            if (Array.isArray(parsed2)) {
              camposCategoria = parsed2.map(c => {
                if (typeof c === 'string') return { key: c.toLowerCase().replace(/\s+/g,'_'), label: c };
                if (c && typeof c === 'object' && c.key) return { key: String(c.key).toLowerCase(), label: c.label || c.key };
                return null;
              }).filter(Boolean);
            ...
            console.error('Erro ao parsear campos da Categoria (fallback):', e);
            camposCategoria = [];
          }
        } else {
          console.log('[EDIT PRODUTO] nenhum "campos" encontrado na tabela Categoria para ID =', produto.Categoria_ID);
        }
        return renderWithCatalogo(camposCategoria);
      });
    } else {
      console.log('[EDIT PRODUTO] produto não possui Categoria_ID (não é possível fallback)');
      // nenhum mapeamento de categoria disponível
      return renderWithCatalogo([]);
    }
  });
});
...
// rota para exibir produto por id (frontend/admin)
...
// rota para exibir produto por slug (compatibilidade)
